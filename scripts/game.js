...
const checkPuzzleToMove = (puzzleToMove, selectedPuzzlePlace) => {
	...
	!!puzzleToMove && canMove && handlePuzzleMove(puzzleToMove, selectedPuzzlePlace.elementIndex);
}

const checkIfPlaceIsEmpty = (puzzlePlace) => {
	return gamePuzzles.every((gamePuzzle) => gamePuzzle.rowIndex !== puzzlePlace.rowIndex || gamePuzzle.columnIndex !== puzzlePlace.columnIndex);
}

const checkIfIsNeighbour = (puzzle, puzzlePlace) => {
	if (Math.abs(puzzle.rowIndex - puzzlePlace.rowIndex) === 1 && puzzle.columnIndex === puzzlePlace.columnIndex) {
		return true
	}
	if (Math.abs(puzzle.columnIndex - puzzlePlace.columnIndex) === 1 && puzzle.rowIndex === puzzlePlace.rowIndex) {
		return true
	}
	return false
}
...
const handlePuzzleMove = (puzzleToMove, index) => {
		...
		isGameOver = gamePuzzles.every((puzzle) => puzzle.elementPositionIndex === puzzle.elementIndex);
		movesCount++;
...
const checkIfGameIsSolvable = () => {
	...
	puzzlesClone.sort((puzzleA, puzzleB) => puzzleA.elementPositionIndex - puzzleB.elementPositionIndex);
...
const setupGame = () => {
	...
	do {
		...
		gamePuzzles.forEach((puzzle) => {
			const index = puzzlePositions.splice(Math.floor((Math.random()*puzzlePositions.length)), 1)[0]
...
const displayInfoScreen = (screenType) => {
	...
	if(screenType === "error") {
		...
		gameOverScreen.querySelector("p").innerHTML = `Liczba ruchów: ${movesCount}`;
